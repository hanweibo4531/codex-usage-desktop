...
$refs = @('System.dll','System.Core.dll','System.Net.Http.dll','System.Web.Extensions.dll','System.Xaml.dll','System.Drawing.dll','System.Windows.Forms.dll','WPF\WindowsBase.dll','WPF\PresentationCore.dll','WPF\PresentationFramework.dll') | ForEach-Object { '/reference:' + (Join-Path $framework $_) }
...
$compilerArgs += @("$PSScriptRoot\WeeklyQuota.cs", "$PSScriptRoot\QuotaPage.cs")
$compilerArgs += @("$PSScriptRoot\RequestSchedule.cs", "$PSScriptRoot\ScheduleDialog.cs", "$PSScriptRoot\ScheduleTests.cs")
