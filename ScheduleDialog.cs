...
static class ScheduleDialog {
 ...
 static TextBox Input(string text){return new TextBox{Text=text,FontSize=14,Margin=new Thickness(0,0,0,12)};}
 public static Window Build(Window owner,RequestSchedule current,Action saved) {
  ...
  body.Children.Add(Label("到点请 Codex 回复一次 OK，使用现有 ChatGPT 登录状态。请求会消耗少量用量，实际重置时间由服务端决定。",12));
   var enabled=new CheckBox{Content="启用每日定时请求",IsChecked=current.Enabled,Margin=new Thickness(0,8,0,18)};body.Children.Add(enabled);
  ...
  body.Children.Add(Label("模型（可选，留空使用 Codex 默认模型）",12));var model=Input(current.Model);body.Children.Add(model);
   var wake=new CheckBox{Content="尝试唤醒睡眠中的电脑",IsChecked=current.WakeComputer,Margin=new Thickness(0,0,0,12)};body.Children.Add(wake);
